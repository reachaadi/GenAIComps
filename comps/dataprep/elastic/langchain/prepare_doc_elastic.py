...
from config import CHUNK_OVERLAP, CHUNK_SIZE, EMBED_MODEL, INDEX_NAME, ES_CONNECTION_STRING

logger = CustomLogger("prepare_doc_elastic")
...
def ingest_doc_to_elastic(doc_path: DocPath):
...
async def ingest_link_to_elastic(link_list: List[str]):
    # Create vectorstore
    if tei_embedding_endpoint:
        # create embeddings using TEI endpoint service
        embedder = HuggingFaceHubEmbeddings(model=tei_embedding_endpoint)
...
@register_microservice(
    name="opea_service@prepare_doc_elastic_file", endpoint="/v1/dataprep/get_file", host="0.0.0.0", port=6011
)
...
@register_microservice(
    name="opea_service@prepare_doc_elastic_del", endpoint="/v1/dataprep/delete_file", host="0.0.0.0", port=6011
)
...
if __name__ == "__main__":
    ...
    opea_microservices["opea_service@prepare_doc_elastic_file"].start()
    opea_microservices["opea_service@prepare_doc_elastic_del"].start()
